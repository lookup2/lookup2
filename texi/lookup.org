...
Dictionaries are indirectly specifed by searc agents.  You may specify
...
dictinaries encoding (UTF-8 is strongly recommended) and
...
agent is basically text searching agent, so you can sepcify either
~ndbuffer~ / ~ndsary~ aget as a ~:backend~.  (You can not use ~ndtext~
...
author has already passed away, but it is still widely used as Latin
dictionary.
