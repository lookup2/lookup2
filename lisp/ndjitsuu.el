...
;; You must prepare index text file (oyaji.txt and jukugo.txt), which
;; should be generated by `jitsuu_mk_oyaji_lst.rb' and
;; `jitsuu_mk_jukugo_lst.rb' program (Shift_JIS encoding), and then
;; convert gaijis to Unicode by `ndjitsuu-convert-gaiji-to-ucs'
;; function equipped in this file.  After that, these two files should
;; be indexed by `mksary' program.
;;
;; Usage:
;;
...
;;         ....
;;         ))
;; (setq ndjitsuu-oyaji-index-file "~/edicts/Jitsuu/oyaji.txt")
;; (setq ndjitsuu-jukugo-index-file "~/edicts/Jitsuu/jukugo.txt")
    ...
    (cond ((string-match "^[0-9]+\\(:[0-9]+\\)?$" string)
          ...
          ((string-match "^[㐀-龻𠀀-𯿽]$" string) ;; １漢字検索
           (let ((file-word (list (cons ndjitsuu-oyaji-index-file 
             ...
             (if (or (equal method 'suffix) (equal method 'substring))
                 (setq file-word (nconc (list (cons ndjitsuu-jukugo-index-file
                                                    (concat string "】")))
                                        file-word)))
          ...
          ((string-match "^[あ-ん]+$" string) ;; かな検索
           (let* ((string (replace-regexp-in-string "ゃ" "や" string))
                  ...
                  (file-word (list (cons ndjitsuu-oyaji-index-file
                                         (concat prefix string suffix))
                                   (cons ndjitsuu-jukugo-index-file
                                         (concat prefix string suffix))
   ...
   (mapcar
    (lambda (file-word)
      (let* ((file (car file-word))
             ...
             (entries
              (split-string entries "\n" t)))
        (when (< (length entries) 200)
          (mapcar
           (lambda (x) 
             (when (string-match "^\\([0-9]+\\(?::[0-9]+\\)?\\) \\(.+\\)$" x)
               (lookup-new-entry
                'regular dictionary (match-string 1 x) (match-string 2 x))))
           entries))))
    file-word ))))
...
Optional argument SUBINDEX indicates JUKUGO index."
